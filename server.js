...
function limitIP(ip) {
  const now = Date.now();
  if (!ipRequests[ip]) { ipRequests[ip] = { count: 1, last: now }; return true; }
  if (now - ipRequests[ip].last > 60000) { ipRequests[ip] = { count: 1, last: now }; return true; }
...
function canRequest(userId) {
  const now = Date.now();
  if (!userLastRequest[userId]) { userLastRequest[userId] = now; return true; }
...
async function pollVideoResult(requestId) {
  for (let i = 0; i < 60; i++) {
    ...
    const status = pollResponse.data?.state || pollResponse.data?.status;

    if (status === "done" || status === "completed") {
      return pollResponse.data?.video?.url || pollResponse.data?.data?.[0]?.url || pollResponse.data?.url;
    }
...
app.post("/generate", async (req, res) => {
  ...
  if (!userData) return res.status(403).json({ error: "Token inválido", details: authError });

  const userId = userData.email || userData.id;
  if (!users[userId]) users[userId] = { credits: DAILY_CREDITS, lastReset: Date.now() };
  const user = users[userId];
  resetCredits(user);

  if (!canRequest(userId)) return res.status(429).json({ error: "Aguarde 3 segundos" });
  if (user.credits < COST_PER_REQUEST) return res.status(403).json({ error: "Sem créditos" });
  ...
  try {
    ...
    if (type === "video") {
      ...
      if (!requestId) return res.status(500).json({ error: "Não foi possível iniciar a geração do vídeo" });

      const videoUrl = await pollVideoResult(requestId);
      user.credits -= COST_PER_REQUEST;
      return res.json({ url: videoUrl, creditsLeft: user.credits, type: "video" });
    }
    ...
    if (type === "animate") {
      ...
      const requestId = createResponse.data?.request_id;
      if (!requestId) return res.status(500).json({ error: "Não foi possível iniciar a animação" });
